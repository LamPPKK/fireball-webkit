...
@main
@MainActor
struct FireballWebKitApp: App {
    @Environment(\.scenePhase) private var scenePhase
    @State private var store: BrowserStore

    init() {
        let uiTesting = ProcessInfo.processInfo.environment["FIREBALL_UI_TESTING"] == "1"
        let cloudKitEnabled = Self.infoBoolean("FireballCloudKitEnabled")
    ...
    private static func makeBlockerUpdater() -> (service: BlockerUpdateService, manifestURL: URL)? {
        let updatesEnabled = infoBoolean("FireballBlockerUpdatesEnabled")
        guard updatesEnabled,
              ...
              let manifestString = Bundle.main.object(forInfoDictionaryKey: "FireballBlockerManifestURL") as? String,
