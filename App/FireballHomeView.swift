...
struct FireballHomeView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 28) {
                ...
                if !profileBookmarks.isEmpty {
                    FireballSectionLabel(title: "Saved coordinates", index: "02")
                ...
                if !profileHistory.isEmpty {
                    FireballSectionLabel(title: "Recent trajectory", index: "03")
                    VStack(spacing: 1) {
        ...
        .background {
            ZStack {
                LinearGradient(
                    colors: [Color.fireballBackground, Color(red: 0.045, green: 0.060, blue: 0.043)],
    ...
    private var hero: some View {
        ViewThatFits(in: .horizontal) {
            HStack(alignment: .center, spacing: 28) {
                heroCopy
                Spacer(minLength: 8)
                meteorVisual.frame(width: 220, height: 210)
            }
            VStack(alignment: .leading, spacing: 22) {
                meteorVisual.frame(maxWidth: .infinity).frame(height: 150)
                heroCopy
            }
        }
        ...
        .background {
            LinearGradient(
                colors: [Color.fireballPanel, Color.fireballRaised.opacity(0.74)],
                startPoint: .topLeading,
                endPoint: .bottomTrailing
            )
        }
    ...
    private var heroCopy: some View {
        VStack(alignment: .leading, spacing: 15) {
            HStack(spacing: 9) {
                Circle().fill(Color.fireballGreen).frame(width: 7, height: 7)
                Text("FIREBALL / WEBKIT 0.1")
                Text("·")
            ...
            Text("Profiles separate website data. Spaces keep tabs organized. Private flights leave no restorable tabs or snapshots.")
    ...
    private var meteorVisual: some View {
        ZStack {
            Circle()
                .stroke(Color.fireballOrange.opacity(0.30), lineWidth: 1)
                .padding(18)
            Circle()
                .stroke(Color.fireballGreen.opacity(0.18), style: StrokeStyle(lineWidth: 1, dash: [4, 8]))
                .padding(42)
            FireballBrandMark(size: dynamicTypeSize.isAccessibilitySize ? 112 : 156)
        }
        .accessibilityElement(children: .ignore)
        .accessibilityLabel("Fireball meteor brand mark")
    }

    private var flightPlan: some View {
        VStack(alignment: .leading, spacing: 12) {
            FireballSectionLabel(title: "Privacy flight plan", index: "01")
            LazyVGrid(columns: flightPlanColumns, spacing: 12) {
                ...
                statusCard(
                    ...
                    accent: store.selectedSpace?.storageMode == .ephemeral ? .fireballOrange : .fireballGreen
                )
                statusCard(
                    ...
                    accent: store.activeProfile?.blockerEnabled == true ? .fireballGreen : .fireballMuted
                )
            }
        }
    }
